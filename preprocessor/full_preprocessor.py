...
INPUT_FRAMES  = 4
TARGET_FRAMES = 1
TOTAL_WINDOW  = INPUT_FRAMES + TARGET_FRAMES   # 5
...
def resize_band(band):
    if band.ndim == 3:
        band = band[0]
    band = band.astype(np.float32)
    return cv2.resize(band, (64, 64), interpolation=cv2.INTER_AREA)
...
def create_sequences():
    ...
    for i in tqdm(range(total_seqs), desc="Creating sequences"):
        out_path = os.path.join(SEQUENCE_DIR, f"seq_{i:03d}.npy")
...
def verify_output():
    ...
    seqs   = sorted(glob(os.path.join(SEQUENCE_DIR,  "*.npy")))
